...
def generate_synthetic_scam_data():
    ...
    scam_categories = ['Phone', 'Email', 'Text', 'In-person', 'Social Media']
    for category in scam_categories:
        df[f'{category.lower()}_scam_rate'] = np.random.normal(df['scam_rate'], 5)
        df[f'{category.lower()}_scam_rate'] = df[f'{category.lower()}_scam_rate'].clip(1, 100)
    ...
    for _, row in scam_data.iterrows():
        city_data = {
            ...
            'categories': {
                ...
                'in_person': round(row['in-person_scam_rate'], 1),
...
def get_city_data(city_name):
    ...
    result = {
        ...
        'demographics': {
            ...
            'elderly_percentage': round(row['elderly_percentage'], 1)
        },
        'analysis': generate_city_analysis(row),
        'safety_recommendations': generate_safety_recommendations(row)
    }

    logger.info(f"Scam data requested and returned for city: {city_name}")
    return jsonify(result)
...
def generate_city_analysis(city_data):
    ...
    categories = ['phone_scam_rate', 'email_scam_rate', 'text_scam_rate',
                  'in-person_scam_rate', 'social_media_scam_rate']
...
def generate_safety_recommendations(city_data):
    ...
    if city_data['in-person_scam_rate'] > 60:
...
if __name__ == '__main__':
    ...
    print(f"Current User's Login: {request.headers.get('X-User-Login', 'Unknown')}")
